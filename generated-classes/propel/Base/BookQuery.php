 ...
 * @method     ChildBookQuery orderByPubDate($order = Criteria::ASC) Order by the pub_date column
 ...
 * @method     ChildBookQuery groupByPubDate() Group by the pub_date column
 ...
 * @method     ChildBook findOneByPubDate(int $pub_date) Return the first ChildBook filtered by the pub_date column
 * @method     ChildBook findOneByPrice(int $price) Return the first ChildBook filtered by the price column
 ...
 * @method     ChildBook requireOneByPubDate(int $pub_date) Return the first ChildBook filtered by the pub_date column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildBook requireOneByPrice(int $price) Return the first ChildBook filtered by the price column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildBook[]|ObjectCollection findByPubDate(int $pub_date) Return ChildBook objects filtered by the pub_date column
 * @method     ChildBook[]|ObjectCollection findByPrice(int $price) Return ChildBook objects filtered by the price column
...
abstract class BookQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, title, isbn, pub_date, price, publisher_id, author_id FROM book WHERE id = :p0';
        try {
    ...
    public function filterByPubDate($pubDate = null, $comparison = null)
    {
        if (is_array($pubDate)) {
            ...
            if (isset($pubDate['min'])) {
                $this->addUsingAlias(BookTableMap::COL_PUB_DATE, $pubDate['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($pubDate['max'])) {
                $this->addUsingAlias(BookTableMap::COL_PUB_DATE, $pubDate['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(BookTableMap::COL_PUB_DATE, $pubDate, $comparison);
    }
